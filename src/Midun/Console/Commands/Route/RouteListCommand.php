    ...
    public function handle()
    {
        ...
        switch ($this->getOptions('format')) {
            case 'array':
                ...
                break;
            case 'json':
                return $this->handleViewJsonFormat($routes);
            default:
                $space15 = $this->makeSpace(15);
                $space25 = $this->makeSpace(25);
                $space5 = $this->makeSpace(5);
                $this->output->print($this->makeSpace(186, self::SPECIFIC_UNDERSCORE));
                $this->output->print(self::SPECIFIC_LINE . "{$space15}uri{$space15}" . self::SPECIFIC_LINE . "{$space25}action{$space25}" . self::SPECIFIC_LINE . "{$space5}method{$space5}" . self::SPECIFIC_LINE . "{$space15}name{$space15}" . self::SPECIFIC_LINE . "{$space15}middlewares{$space15}" . self::SPECIFIC_LINE);
                $this->output->print(self::SPECIFIC_LINE . $this->makeSpace(184, self::SPECIFIC_MIDDLESCORE) . self::SPECIFIC_LINE);
                foreach ($routes as $route) {
                    ...
                    $this->output->print(self::SPECIFIC_LINE . $uri . self::SPECIFIC_LINE . $action . self::SPECIFIC_LINE . $method . self::SPECIFIC_LINE . $name . self::SPECIFIC_LINE . $middlewares . self::SPECIFIC_LINE);
                }
                $this->output->print($this->makeSpace(186, self::SPECIFIC_MIDDLESCORE));
        }
    }
